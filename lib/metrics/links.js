...
function getHostname(source) {
  try {
    ...
    let anchorUrl = url.parse(source);

    return _.get(anchorUrl, 'hostname');
  } catch (err) {
    return null;
  }
}

function tagSourceMapping() {
  // TODO: Support for tags picture, figure
  return {
    'a': 'href',
  ...
  return new Promise((resolve, reject) => {
    scrapper.html(stareDocument.link)
      .then(data => {
        ...
        let tagMapping = tagSourceMapping();
        let anchors = []
        let hostname = '';
        let tag, source, attrs;
        
        anchors.push(getHostname(stareDocument.link));

        $('a, img, video, audio, iframe, source').each((i, el) => {
          tag = _.get(el, 'name');
          source = _.get(el, ['attribs', tagMapping[tag]]);
