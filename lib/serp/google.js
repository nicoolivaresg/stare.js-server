...
if (_.isEmpty(global.stareOptions.googleApiKey)) {
  throw `You must define your Google API KEY (GOOGLE_API_KEY) on your .env file to continue.`;
}

if (_.isEmpty(global.stareOptions.googleApiCx)) {
...
function getResultPages(query, numberOfResults) {
  ...
  const opts = {
    ...
    start: 0,
    num: numberOfResults || global.stareOptions.numberOfResults  // per API rules, must be between [1, 10]
  };

  return new Promise((resolve, reject) => {
    customsearch.cse.list(opts)
      .then(response => {
        let googleResult = _.get(response, 'data');
